...
let rec match_expressions lhs expr =
    match (lhs, expr) with
    | (Identifier id, _) -> 
        Some (Substitution.singleton id expr)
    ...
    | (Colon (l1, l2), Colon (e1, e2)) ->
        ...
        | (Some subst1, Some subst2) -> 
            Some (Substitution.merge (fun _ a b -> match b with None -> a | Some _ -> b) subst1 subst2)
        | (Some subst, None) | (None, Some subst) -> Some subst
        | _ -> None)
    | _ -> None
...
let perform_proof_step equality goal =
    let transformed_goal = attemptRewrite equality goal in
    match transformed_goal with
    | Some new_goal ->
        print_endline ("Goal: " ^ string_of_expression goal);
        print_endline ("Used Equality: " ^ string_of_equality equality);
        (* Assuming you have a way to retrieve and print the substitutions made *)
        print_endline ("Transformed Goal: " ^ string_of_expression new_goal);
        transformed_goal
    | None ->
